...
export async function showCargo(game) {
    ...
    try {
        const data = await SFApi.Cargo.getCargo(selectedUnit.id, game.userId);
        ...
        if (adjacentObjects.length > 0) {
            ...
            adjacentObjects.forEach(obj => {
                const btn = document.createElement('button'); btn.className = 'transfer-target-btn';
                btn.innerHTML = `${getUnitIcon(obj.type)} ${obj.meta.name || obj.type} (${obj.x}, ${obj.y})`;
                btn.dataset.action = 'open-transfer'; btn.dataset.fromId = String(selectedUnit.id); btn.dataset.toId = String(obj.id); btn.dataset.toName = obj.meta.name || obj.type;
    ...
    } catch (error) { console.error('Error getting cargo:', error); game.addLogEntry('Failed to get cargo information', 'error'); }
}

export async function showTransferModal(game, fromObjectId, toObjectId, toObjectName) {
    try {
        const [fromData, toData] = await Promise.all([
            SFApi.Cargo.getCargo(fromObjectId, game.userId),
            SFApi.Cargo.getCargo(toObjectId, game.userId)
        ]);
        ...
        fromCargo.items.forEach(item => {
            const transferItem = document.createElement('div'); transferItem.className = 'transfer-item'; const inputId = `transfer-${item.resource_name}`;
        ...
        if (toCargo.items && toCargo.items.length > 0) {
            ...
            toCargo.items.forEach(item => {
                const transferItem = document.createElement('div'); transferItem.className = 'transfer-item'; const inputId = `transfer-from-${item.resource_name}`;
                transferItem.innerHTML = `
                    <div class="transfer-item-info"><span class="cargo-icon" style="color: ${item.color_hex}">${item.icon_emoji}</span>
                    <div class="transfer-details"><div class="transfer-name">${item.resource_name}</div><div class="transfer-available">Available: ${item.quantity} units</div></div></div>
                    <div class="transfer-controls"><input type="number" class="transfer-quantity" min="1" max="${item.quantity}" value="1" id="${inputId}">
        ...
        transferDisplay.addEventListener('click', (e) => {
            const btn = e.target.closest('[data-action="transfer"]'); if (!btn) return;
            const fromId = Number(btn.dataset.fromId); const toId = Number(btn.dataset.toId); const res = btn.dataset.resource; const inputId = btn.dataset.inputId; const toName = btn.dataset.toName || 'Target';
            const qty = document.getElementById(inputId)?.value; performTransfer(game, fromId, toId, res, qty, toName);
        });
    ...
    } catch (error) { console.error('Error showing transfer modal:', error); game.addLogEntry('Failed to show transfer options', 'error'); }
}

export async function performTransfer(game, fromObjectId, toObjectId, resourceName, quantity, toObjectName) {
    const transferQuantity = parseInt(quantity);
    if (!transferQuantity || transferQuantity <= 0) { game.addLogEntry('Invalid transfer quantity', 'warning'); return; }
    try {
        const result = await SFApi.Cargo.transfer(parseInt(fromObjectId), parseInt(toObjectId), resourceName, transferQuantity, game.userId);
        if (result && result.success) {
            game.addLogEntry(`Successfully transferred ${transferQuantity} ${resourceName} to ${toObjectName}`, 'success');
            window.UI.closeModal();
            if (game.selectedUnit && game.selectedUnit.id === fromObjectId) { setTimeout(() => showCargo(game), 100); }
        } else { game.addLogEntry(result?.error || 'Transfer failed', 'error'); }
    } catch (error) { console.error('Error performing transfer:', error); game.addLogEntry(error?.data?.error || 'Failed to transfer resources', 'error'); }
}

export async function updateCargoStatus(game, shipId) {
    try {
        const data = await SFApi.Cargo.getCargo(shipId, game.userId);
        if (data) {
