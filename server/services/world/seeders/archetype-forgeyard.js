// Capital Forgeyard — “Hull & Bulwark” STUB
const { randInt, randFloat, choice } = require('../rng');

const MINERALS = {
    primary: ['Starforged Carbon','Luminite'],
    secondary: ['Magnetrine','Gravium','Heliox Ore','Quarzon','Cryphos']
};

const DISPLAY = { name:'Capital Forgeyard', 
    description:'Hull and bulwark; heavy mass limits and cap boosts.' 
};

function plan({ sectorId, seed, rng }) {
    const regions = { grid: [['A','B','B'],['A','B','B'],['A','C','B']], health: { A: 55, B: 55, C: 55 } };
    const sun = { x:2500, y:2500 };
    const planets=[]; const n=choice(rng,[4,5,6]); for(let i=0;i<n;i++){ const r=randInt(rng,900,2200); const a=randFloat(rng,0,Math.PI*2); planets.push({id:`P${i}`,x:2500+Math.cos(a)*r,y:2500+Math.sin(a)*r}); }
    const belts=[];
...
async function persist({ sectorId, plan, db }) {
    ...
    const metaSun=JSON.stringify({name:'Forge Star', celestial:true, alwaysKnown:1});
