// Dark Nebula Nursery — “Shadows & Birth” STUB
const { randInt, randFloat, choice } = require('../rng');

const MINERALS = {
    primary: ['Spectrathene','Voidglass'],
    secondary: ['Nebryllium','Oblivium','Aetherium','Quarzon','Luminite']
};

const DISPLAY = { name:'Dark Nebula Nursery', 
    description:'Shadows and birth—stealth and decoys thrive.' 
};

function plan({ sectorId, seed, rng }) {
    const regions = { grid: [['A','B','B'],['A','C','B'],['A','B','A']], health: { A: 55, B: 55, C: 55 } };
    const sun = { x:2500, y:2500 };
    const planets=[]; const n=choice(rng,[3,4]); for(let i=0;i<n;i++){ const r=randInt(rng,900,2100); const a=randFloat(rng,0,Math.PI*2); planets.push({id:`P${i}`,x:2500+Math.cos(a)*r,y:2500+Math.sin(a)*r}); }
    const belts=[];
...
async function persist({ sectorId, plan, db }) {
    ...
    const metaSun=JSON.stringify({name:'Dark Star', celestial:true, alwaysKnown:1});
