// Ion Tempest — “EM Weather” STUB
const { randInt, randFloat, choice } = require('../rng');

const MINERALS = {
    primary: ['Cryphos','Luminite'],
    secondary: ['Auralite','Magnetrine','Solarite','Nebryllium','Phasegold']
};

const DISPLAY = { name:'Ion Tempest', 
    description:'EM weather rails and bypasses define safe routes.' 
};

function plan({ sectorId, seed, rng }) {
    const regions = { grid: [['A','B','A'],['B','C','B'],['A','B','A']], health: { A: 55, B: 55, C: 55 } };
    const sun = { x:2500, y:2500 };
    const planets=[]; const n=choice(rng,[4,5]); for(let i=0;i<n;i++){ const r=randInt(rng,700,2000); const a=randFloat(rng,0,Math.PI*2); planets.push({id:`P${i}`,x:2500+Math.cos(a)*r,y:2500+Math.sin(a)*r}); }
    const belts=[];
...
async function persist({ sectorId, plan, db }) {
    ...
    const metaSun=JSON.stringify({name:'Ion Star', celestial:true, alwaysKnown:1});
