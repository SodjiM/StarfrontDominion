// Starlight Relay Network — “Beacons & Slipstreams” STUB
const { randInt, randFloat, choice } = require('../rng');

const MINERALS = {
    primary: ['Aetherium','Fluxium'],
    secondary: ['Tachytrium','Quarzon','Heliox Ore','Phasegold','Riftstone']
};

const DISPLAY = { name:'Starlight Relay Network', 
    description:'Beacons and slipstreams enable priority lanes.' 
};

function plan({ sectorId, seed, rng }) {
    const regions = { grid: [['A','A','B'],['A','B','B'],['A','C','B']], health: { A: 55, B: 55, C: 55 } };
    const sun = { x:2500, y:2500 };
    const planets=[]; const n=choice(rng,[4,5,6]); for(let i=0;i<n;i++){ const r=randInt(rng,700,2100); const a=randFloat(rng,0,Math.PI*2); planets.push({id:`P${i}`,x:2500+Math.cos(a)*r,y:2500+Math.sin(a)*r}); }
    const belts=[];
...
async function persist({ sectorId, plan, db }) {
    ...
    const metaSun=JSON.stringify({name:'Relay Star', celestial:true, alwaysKnown:1});
