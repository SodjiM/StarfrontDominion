// Cryo Comet Rain — “Cold Logistics” STUB
const { randInt, randFloat, choice } = require('../rng');

const MINERALS = {
    primary: ['Kryon Dust','Heliox Ore'],
    secondary: ['Mythrion','Luminite','Auralite','Fluxium','Starforged Carbon']
};

const DISPLAY = { name:'Cryo Comet Rain', 
    description:'Cold logistics; icing windows and safe nets.' 
};

function plan({ sectorId, seed, rng }) {
    const regions = { grid: [['A','B','B'],['A','A','B'],['A','C','B']], health: { A: 55, B: 55, C: 55 } };
    const sun = { x:2500, y:2500 };
    const planets=[]; const n=choice(rng,[4,5]); for(let i=0;i<n;i++){ const r=randInt(rng,900,2300); const a=randFloat(rng,0,Math.PI*2); planets.push({id:`P${i}`,x:2500+Math.cos(a)*r,y:2500+Math.sin(a)*r}); }
    const belts=[];
...
async function persist({ sectorId, plan, db }) {
    ...
    const metaSun=JSON.stringify({name:'Cold Star', celestial:true, alwaysKnown:1});
