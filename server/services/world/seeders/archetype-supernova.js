// Supernova Remnant — “Furnace & Shrapnel” STUB
const { randInt, randFloat, choice } = require('../rng');

const MINERALS = {
    primary: ['Drakonium','Corvexite'],
    secondary: ['Pyronex','Solarite','Magnetrine','Gravium','Starforged Carbon']
};

const DISPLAY = { name:'Supernova Remnant', 
    description:'Furnace and shrapnel; hazardous, rich debris lanes.' 
};

function plan({ sectorId, seed, rng }) {
    const regions = { grid: [['A','B','B'],['B','C','B'],['A','B','A']], health: { A: 55, B: 55, C: 55 } };
    const sun = { x:2500, y:2500 };
    const planets=[]; const n=choice(rng,[3,4,5]); for(let i=0;i<n;i++){ const r=randInt(rng,800,2200); const a=randFloat(rng,0,Math.PI*2); planets.push({id:`P${i}`,x:2500+Math.cos(a)*r,y:2500+Math.sin(a)*r}); }
    const belts=[];
...
async function persist({ sectorId, plan, db }) {
    ...
    const metaSun=JSON.stringify({name:'Remnant Star', celestial:true, alwaysKnown:1});
