// Solar Flare Engine — “Light & Rhythm” STUB
const { randInt, randFloat, choice } = require('../rng');

const MINERALS = {
    primary: ['Solarite','Pyronex'],
    secondary: ['Luminite','Cryphos','Auralite','Tachytrium','Corvexite']
};

const DISPLAY = { name:'Solar Flare Engine', 
    description:'Light and rhythm govern windows and protection.' 
};

function plan({ sectorId, seed, rng }) {
    const regions = { grid: [['A','A','B'],['B','C','B'],['A','B','B']], health: { A: 55, B: 55, C: 55 } };
    const sun = { x:2500, y:2500 };
    const planets=[]; const n=choice(rng,[5,6]); for(let i=0;i<n;i++){ const r=randInt(rng,600,2200); const a=randFloat(rng,0,Math.PI*2); planets.push({id:`P${i}`,x:2500+Math.cos(a)*r,y:2500+Math.sin(a)*r}); }
    const belts=[];
...
async function persist({ sectorId, plan, db }) {
    ...
    const metaSun=JSON.stringify({name:'Flare Star', celestial:true, alwaysKnown:1});
