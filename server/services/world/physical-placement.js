...
async function physicalObjects(db,sectorId,bounds=null) {
    const objectSql=bounds
        ? 'SELECT * FROM sector_objects WHERE sector_id=? AND x BETWEEN ? AND ? AND y BETWEEN ? AND ?'
        : 'SELECT * FROM sector_objects WHERE sector_id=?';
    const objectArgs=bounds?[sectorId,bounds.minX,bounds.maxX,bounds.minY,bounds.maxY]:[sectorId];
    const nodeSql=bounds
        ? 'SELECT id,x,y FROM resource_nodes WHERE sector_id=? AND is_depleted=0 AND x BETWEEN ? AND ? AND y BETWEEN ? AND ?'
        : 'SELECT id,x,y FROM resource_nodes WHERE sector_id=? AND is_depleted=0';
    const nodeArgs=bounds?[sectorId,bounds.minX,bounds.maxX,bounds.minY,bounds.maxY]:[sectorId];
    const objects=await query(db,objectSql,objectArgs);
    const nodes=await query(db,nodeSql,nodeArgs);
...
async function placeNear(db,sectorId,mover,host,{anchored=false,maxRadius=100}={}) {
    ...
    const extent=nav.scale.extent(host)+nav.scale.extent(mover)+minRadius+maxRadius+2;
    const objects=await physicalObjects(db,sectorId,{minX:Math.floor(host.x-extent),maxX:Math.ceil(host.x+extent),minY:Math.floor(host.y-extent),maxY:Math.ceil(host.y+extent)});
